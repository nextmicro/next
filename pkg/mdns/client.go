		...
		case m := <-msgCh:
			...
			if e.complete() {
				...
				if err := client.sendQuery(m); err != nil {
					logger.Errorf("[mdns] failed to query instance %s: %v", e.Name, err)
				}
			}
		}
	}
...
func newClient() (*client, error) {
	...
	if err4 != nil && err6 != nil {
		logger.Errorf("[mdns] failed to bind to udp port: %v %v", err4, err6)
	}
	...
	for {
		...
		case resp := <-msgCh:
			...
			if inp.complete() {
				...
				m.RecursionDesired = false
				if err := c.sendQuery(m); err != nil {
					logger.Errorf("[mdns] failed to query instance %s: %v", inp.Name, err)
				}
			}
		case <-params.Context.Done():
			return nil
